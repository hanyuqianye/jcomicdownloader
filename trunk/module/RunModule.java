...
public class RunModule
{
                                             ...
                                             String urlString )
    {
        ...
        if ( parse.isSingleVolumePage( urlString ) )
        { // ex. http://www.89890.com/comic/7953/
    ...
    private synchronized void runSingle( ParseOnlineComicSite parse, String urlString, String titleString )
    {
        ...
        if ( Run.isAlive )
        {
            ...
            if ( parse.siteID == Site.EH || parse.siteID == Site.EX /*
                     ...
                     || ( parse.siteID == Site.CK_NOVEL && parse.getTitle().equals( parse.getWholeTitle() ) )
                     // EYNY小說規則：唯有當下載單一作品時，才用一層資料夾（此時標題名稱與集數名稱相同）
                     || ( parse.siteID == Site.EYNY_NOVEL && parse.getTitle().equals( parse.getWholeTitle() ) )
    ...
    public synchronized void followingWork( ParseOnlineComicSite parse )
    { // compress or not & delete or not
        Common.debugPrintln( "開始後續動作：" );
        if ( Run.isAlive )
        {
            File downloadPath = new File( parse.getDownloadDirectory() );

            if ( SetUp.getRunSingleDoneScript() )
            { // 每一集下載完就執行單一任務腳本
            ...
            {
                if ( SetUp.getAutoCompress() )
                { // compress to zip file or not
                ...
                if ( SetUp.getDeleteOriginalPic() )
                { // delete the whole folder or not
