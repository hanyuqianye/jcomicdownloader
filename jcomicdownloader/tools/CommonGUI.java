...
public class CommonGUI {
    ...
    public String[] getClassNames() {
        ...
        int allClassAmount = defaultClassNames.length + jtattooClassNames.length + nimRodClassNames.length;
        ...
        for ( int i = 0 ; i < nimRodClassNames.length ; i++ ) {
            allClassNames[count++] = nimRodClassNames[i];
        }
    ...
    public String[] getSkinStrings() {
        ...
        int allSkinsAmount = defaultSkinStrings.length + jtattooSkinStrings.length + nimRodSkinStrings.length;
        ...
        for ( int i = 0 ; i < nimRodSkinStrings.length ; i++ ) {
            allSkinStrings[count++] = nimRodSkinStrings[i];
        }
    ...
    // 下載JTattoo.jar檔
    public void downloadNewTheme( String themeName, final String themeFileName, final String themeURL ) {
        String skinName = getSkinNameFromClassName( SetUp.getSkinClassName() );
        int choice = JOptionPane.showConfirmDialog( ComicDownGUI.mainFrame, "資料夾內未發現" + themeFileName + "，無法使用"
                + skinName + "界面！\n\n請問是否要下載" + themeFileName + " ？",
        ...
        if ( choice == JOptionPane.YES_OPTION ) {
            Thread downThread = new Thread( new Runnable() {
                ...
                public void run() {
                    ...
                    String fileURL = themeURL;
                    Common.downloadFile( fileURL, Common.getNowAbsolutePath(), themeFileName, false, "" );

                    Run.isAlive = backupValue; // 還原原值

                    JOptionPane.showMessageDialog( ComicDownGUI.mainFrame, themeFileName + "下載完畢，程式即將關閉，請您再次啟動",
            ...
            JOptionPane.showMessageDialog( ComicDownGUI.mainFrame, "不下載" + themeFileName + "，使用預設的"
                    + skinName + "界面",
                    "提醒訊息", JOptionPane.INFORMATION_MESSAGE );
