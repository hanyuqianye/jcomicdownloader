...
public class ParseBlogspot extends ParseOnlineComicSite {
    ...
    public void parseComicURL() { // parse URL and save all URLs in comicURL  //
        ...
        for ( int p = 0 ; p < totalPage && Run.isAlive ; ) {
            ...
            for ( int j = 0 ; j < nowPageCount && p < totalPage && Run.isAlive ; j++ ) {
                ...
                articleTitle = replaceProcess( articleTitle ); // 拿掉html編碼的字元
                articleTitle = Common.getStringRemovedIllegalChar( 
                        Common.getTraditionalChinese( articleTitle ) ); // 轉為繁體字

                titles[p] = articleTitle + ".txt";
                Common.debugPrint( (p + 1) + " " + titles[p] ); // debug

                // 每解析一個網址就下載一張圖
                if ( !new File( getDownloadDirectory() + titles[p] ).exists() ) {
                    singlePageDownload( getTitle(), getWholeTitle(), comicURL[p], totalPage, p + 1, 0 );
                    String fileName = formatter.format( p + 1 ) + ".html";
                    hadleWholeNovel( fileName, titles[p] );  // 處理小說主函式
                }
                else {
                    Common.debugPrintln( titles[p] + "已下載，跳過" );
                }
    ...
    // 處理小說主函式
    public void hadleWholeNovel( String fileName, String title ) {
        ...
        allNovelText = Common.getTraditionalChinese( allNovelText ); // 轉為繁體字
    ...
    public List<List<String>> getVolumeTitleAndUrlOnMainPage( String urlString, String allPageString ) {
        ...
        beginIndex = allPageString.indexOf( "id='Label1'" );
        if ( beginIndex > 0 ) { // 代表有標籤分類
            ...
            for ( int i = 0 ; i < labelCount ; i++ ) {
                ...
                // 取得此標籤有幾篇文章
                beginIndex = tempString.indexOf( "dir='ltr'", beginIndex );
                beginIndex = tempString.indexOf( ">", beginIndex ) + 1;
                ...
                volumeTitle = replaceProcess( volumeTitle ); // 拿掉html編碼的字元
