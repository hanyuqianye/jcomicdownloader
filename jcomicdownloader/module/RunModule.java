...
public class RunModule {
    ...
    public synchronized void runMainProcess( ParseOnlineComicSite parse, 
        ...
        if ( parse.isSingleVolumePage( urlString ) ) { // ex. http://www.89890.com/comic/7953/
            ...
            List<List<String>> combinationList = parse.getVolumeTitleAndUrlOnMainPage( urlString, allPageString );
    ...
    private synchronized void runSingle( ParseOnlineComicSite parse, String urlString, String titleString ) {
        if ( Run.isAlive ) {
            ...
            //if ( !SetUp.assignDownloadPath )
            //    SetUp.setDownloadPath( parse.getDefaultDownloadPath() ); // set download directory

            if ( parse.getSiteID() != Site.EH && SetUp.getDownloadPicFile() ) {
        ...
        if ( Run.isAlive ) {
            ...
            if ( SetUp.getAutoCompress() ) { // compress to zip file or not
                ...
                if ( !zipFile.exists() ) {
                    ...
                    System.out.println( zipFile.getAbsolutePath() + " already made!" );
            }
